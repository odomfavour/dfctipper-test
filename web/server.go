...
func NewServer(cfg Config, mux *chi.Mux) (*Server, error) {
	...
	templates := NewTemplates(cfg.Viewsfolder, cfg.ReloadHTML, commonTemplates, MakeTemplateFuncMap())
	templates.AddTemplate("status")

	authCommonTemplates := []string{"auth_layout"}
	authTemplates := NewTemplates(cfg.Viewsfolder, cfg.ReloadHTML, authCommonTemplates, MakeTemplateFuncMap())
	authTemplates.AddTemplate("status")

	siteCommonTemplates := []string{"site_layout"}
	siteTemplates := NewTemplates(cfg.Viewsfolder, cfg.ReloadHTML, siteCommonTemplates, MakeTemplateFuncMap())
	siteTemplates.AddTemplate("status")
...
func (s *Server) MountAssetPaths(pathPrefix string, publicFolder string) {
	...
	FileServer(s.webMux, pathPrefix+"site", publicFolder+"site", s.cfg.CacheControlMaxAge)
}
